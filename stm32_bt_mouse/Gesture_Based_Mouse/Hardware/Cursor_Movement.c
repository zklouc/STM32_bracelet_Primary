...
#define DT          0.01f// 采样时间间隔 (1.0f/SAMPLE_RATE)
...
// 灵敏度参数
#define SENSITIVITY 400.0f  // 光标移动灵敏度

//按键部分
#define Key_THRESHOLD          0.05f  // 加速度阈值（g）
#define Key_VELOCITY_THRESHOLD 0.1f // 速度阈值（m/s）
// 状态变量
...
// 初始化时采集100次数据，计算各轴零偏（含重力影响）
//静止时Z轴加速度≈1g（重力），但X/Y轴理论上应为0。
//校准目标：消除零偏（Offset），而不仅仅是初始值。
    ...
    if(Mode_Flag==0)
		{
			...
			if((filtered_ax>Key_THRESHOLD)&& (velocity_x >Key_VELOCITY_THRESHOLD))
			{
				Key_Num|=0x02;
				key_delay_falg=1;
			...
			else if((filtered_ax<(-Key_THRESHOLD))&& (velocity_x <-Key_VELOCITY_THRESHOLD)){
				Key_Num|=0x01;
				key_delay_falg=1;
				TIM_SetCounter(TIM1,0);//清除计数
		    TIM_ITConfig(TIM1, TIM_IT_Update, ENABLE);					//开启TIM1的更新中断
